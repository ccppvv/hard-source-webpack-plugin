...
const generateRaw = fs.readFileSync(
  path.join(__dirname, '_generate.js'),
  'utf8'
);
const generateBlocks = generateRaw
  ...
  .reduce(
    (carry, item, index) => (index % 2 === 0 ? [...carry, item] : carry),
    []
  );
...
for (const dependency of dependencyInfo) {
  ...
  output.push(
    `const ${dependency[0]} = require('webpack/lib/dependencies/${
      dependency[0]
    }');`
  );
  ...
  if (
    DepName === 'AMDDefineDependency' ||
    DepName === 'LocalModuleDependency'
  ) {
    output.push(``);
